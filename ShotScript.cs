...
public class ShotScript : MonoBehaviour
{
	...
	void FixedUpdate() // Update is called once per frame
	{
		...
		relativeVelocityBallToAir = (velocity - windVelocity).magnitude * Vector3.Normalize(velocity-windVelocity);
	...
	void OnCollisionEnter(Collision collision)
	{
		...
		Debug.Log("touch " + collision.gameObject + normal);

		/* Handle "bouncing" behavior here. */
		if (!myMap.ContainsKey(collision.rigidbody) || myMap[collision.rigidbody].timeSinceLastCollision_ > 0.35) { // This works sort of like a timer, so that when bounces become shorter than 0.35s, the ball will not be rolling anymore.
			Vector3 vNormProj = Vector3.Project(velocity, normal); // Calculate the velocity projected onto the normal
			float impactMass = collision.rigidbody.mass;
			velocity -= vNormProj; // Set velocity to the vector rejection of the velocity onto the normal.
			velocity += vNormProj * (0.046f - 0.786f * impactMass) / (0.046f + impactMass);
			bouncingTest = true;
		}

		myMap[collision.rigidbody] = new CollisionHolder(collision, 0f, bouncingTest, touchingTest, normal);
	}
